...
Uses urllib.
What's my status? #1
4-hbtn-status.py: Python script that fetches https://intranet.hbtn.io/status.
Uses requests.
...
Uses requests.
My Github! -10-my_github.py: Python script that takes GitHub credentials (username and password) and uses the Github API to display the corresponding ID.
Usage: ./10-my_github.py <username> <password>
Uses requests.
Time for an interview!
100-github_commit.py: Python script that lists the 10 most recent comments of a given GitHub repository using the GitHub API.
